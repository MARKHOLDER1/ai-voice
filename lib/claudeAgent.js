...
const { transferCall } = require('./callTransfer');
...
5. If the caller is upset, frustrated, or explicitly asks for a person, offer to transfer them to Mark using the transfer_to_human tool. Tell them you're connecting them now — the transfer happens immediately once you call the tool.
...
const tools = [
  ...
  },
  {
    ...
    description: 'Immediately transfers the live call to Mark\'s phone. This actually moves the call — use it only when the caller explicitly asks for a person, is upset, or has a request outside what you can help with.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  switch (name) {
    ...
    case 'transfer_to_human':
      if (!TRANSFER_PHONE_NUMBER) {
        return { transferred: false, error: 'No transfer number configured.' };
      }
      try {
        await transferCall(context.callSid, TRANSFER_PHONE_NUMBER);
 ...
 */
async function getAssistantReply(messageHistory, context = {}) {
